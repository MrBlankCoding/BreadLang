...
// Simple range function for range(n) - creates [0, 1, 2, ..., n-1]
BreadArray* bread_range(int n) {
    return bread_range_create(0, n, 1);
}

// LLVM backend wrapper for bread_array_get
int bread_array_get_value(BreadArray* a, int idx, BreadValue* out) {
    if (!out) return 0;
    
    // Initialize output to nil first
    bread_value_set_nil(out);
...
// LLVM backend wrapper for getting array element from BreadValue
...
// LLVM backend wrapper for getting array length from BreadValue  
