...
static int estimate_instruction_count(ASTStmt* stmt) {
    ...
    switch (stmt->kind) {
        case AST_STMT_VAR_DECL:
            count += 2; // alloca + store
            if (stmt->as.var_decl.init) {
                count += 3; // expression evaluation
            }
            break;
        ...
        case AST_STMT_PRINT:
            count += 5; // function call overhead
            break;
            
        case AST_STMT_IF:
            count += 2; // branch instruction
        ...
        case AST_STMT_RETURN:
            count += 1; // return instruction
            break;
            
        case AST_STMT_EXPR:
            count += 2; // expression evaluation
            break;
            
        case AST_STMT_BREAK:
        case AST_STMT_CONTINUE:
            count += 1; // branch instruction
            break;
...
static int count_function_calls(ASTStmt* stmt) {
    ...
    switch (stmt->kind) {
        case AST_STMT_PRINT:
            calls += 1; // print is a function call
            break;
...
static void analyze_stmt_optimization(ASTStmt* stmt) {
    ...
    // Allocate hints for this statement
