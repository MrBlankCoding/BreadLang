...
ExprResult evaluate_binary_op(ExprResult left, ExprResult right, char op) {
        ...
        if (left.type == TYPE_DOUBLE && right.type == TYPE_DOUBLE) {
            ...
            switch (op) {
                ...
                case '%':
                    ...
                    return create_error_result();
                default: return create_error_result();
            }
            return create_double_result(result_val);
            ...
            return create_int_result(result_val);
        } else {
    ...
    if (op == '=' || op == '!' || op == '<' || op == '>') {
        ...
        return create_bool_result(result_val);
    }
...
ExprResult evaluate_unary_op(ExprResult operand, char op) {
    if (op == '!') {
        ...
        return create_bool_result(!operand.value.bool_val);
    }
