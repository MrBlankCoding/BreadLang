...
typedef struct BreadObjectNode {
    void* object;
    BreadObjKind kind;
    int marked;  // For mark-and-sweep cycle collection
    struct BreadObjectNode* next;
} BreadObjectNode;

typedef struct {
    BreadObjectNode* all_objects;  // Linked list of all heap objects
    BreadMemoryStats stats;
    int cycle_collection_enabled;
    int cycle_collection_threshold;  // Trigger collection after N allocations
    int allocations_since_gc;
    int debug_mode;  // Enable detailed debugging output
} BreadMemoryManager;
